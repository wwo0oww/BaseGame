...
func (self *Wrapper) GetDrv()*sql.DB{
	return self.drv
}

func (self *Wrapper) QueryOne(query string, args ...interface{})*Wrapper{
	if self.drv == nil {
	...
	self.OneRow = self.drv.QueryRow(query, args...)
...
func (self *Wrapper) OneScan(dest ...interface{}) {

	self.Err = self.OneRow.Scan(dest...)
